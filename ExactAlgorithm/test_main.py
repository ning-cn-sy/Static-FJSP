...
# @Software: PyCharm 
# @Comment :
from Config.Milp import milp_variable, milp_constraint
from Config.Task import Task
from Solver.CPLEX.Cplex_Solver import cplex_solver
from Util.draw_util import draw_gantt_chart
from Util.generate import read_info, generate_data_
from Util.name_util_old import t_name, c_name, x_name, y_name, h_name, s_name, constraint_name, spilt_variable_name
...
job_list, machine_list, auxiliary_module_list = read_info("D:\PhD\Research\PythonResearch\pythonProject\ExactAlgorithm\Example\MFJS01.fjs")
    ...
    for op in job.op_list:
        ...
        for machine in machine_list:
            ...
            for am in auxiliary_module_list:
                decision_variables[3].append(milp_variable(y_name(job.id, op.id, machine.id, am.id), 1, 0, type="I"))
        for _job in job_list:
            for _op in _job.op_list:
                if _op != op:
                    decision_variables[4].append(milp_variable(h_name(job.id, op.id, _job.id, _op.id), 1, 0, type="I"))
                    decision_variables[5].append(milp_variable(s_name(job.id, op.id, _job.id, _op.id), 1, 0, type="I"))
# 准备目标
obj = []
                    ...
                    for _op in list(set(_job.op_list) - set(list([op]))):
                        ...
                        for _am in auxiliary_module_list:
                            ...
                            constraint10.append(milp_constraint(
                                ...
                                rhs=get_process_time(op, machine, am) + get_reconfigure_time_γ(am, _am) - 3.0 * W,
                                sense="G"
                            ))
                            # 准备约束11
                            constraint11.append(milp_constraint(
                                name=constraint_name(11, job_id, op_id, machine_id, am_id, _job_id, _op_id, _am.id, 0),
                                variables_name=[c_name(_job_id, _op_id)] + [c_name(job_id, op_id)] + [
                                    y_name(job_id, op_id, machine_id, am.id)] + [
                                                   y_name(_job_id, _op_id, machine_id, _am_id)] + [
                                                   h_name(job_id, op_id, _job_id, _op_id)],
                                weights=[1.0] + [-1.0] + [-W] + [-W] + [W],
                                rhs=get_process_time(_op, machine, _am) + get_reconfigure_time_γ(_am, am) - 2.0 * W,
                                sense="G"))
                if am_id != 0:
                    # 准备约束12
                    for _job in job_list:
                        ...
                        for _op in list(set(_job.op_list) - set(list([op]))):
                            ...
                            for _machine in machine_list:
                                ...
                                constraint12.append(milp_constraint(
                                    ...
                                    weights=[1.0] + [-1.0] + [-W] + [-W] + [-W],
                                    rhs=get_process_time(op, machine, am) + get_reconfigure_time_τ(machine, _machine,
                                                                                                   am) - 3.0 * W,
                                    sense="G"))
                                # 准备约束13
