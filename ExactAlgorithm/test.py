...
from Util.name_util_old import t_name, c_name, x_name, y_name, h_name, s_name, constraint_name, spilt_variable_name, b_name, \
    Y_name
...
job_list, machine_list, auxiliary_module_list = read_info(f"D:\pythonProject\ExactAlgorithm\Example\\{case}.fjs")
...
for job in job_list:
    for op in job.op_list:
        decision_variables[0].append(milp_variable(b_name(job.id, op.id), type="C"))
        for machine in machine_list:
            decision_variables[1].append(milp_variable(x_name(job.id, op.id, machine.id), 1, 0, type="I"))
        for _job in job_list:
            for _op in _job.op_list:
                if _op != op:
                    decision_variables[4].append(
                        milp_variable(Y_name(job.id, op.id, _job.id, _op.id), 1, 0, type="I"))
...
for index, job in enumerate(job_list):
    ...
    # G 大于号
    constraint1.append(
        milp_constraint(constraint_name(1, job_id), [decision_variables[-1][0], [b_name(job_id, op_len)] +
                                                     [x_name(job_id, op_len, machine.id) for machine in machine_list]],
                        [1.0, -1.0, [-machine[1] for machine in job.op_list[-1].available_machines]],
                        0, "G"))
    constraint2.append(
...
solver = cplex_solver(variables, constraints, obj, "MIN", 3600)
# 开始计算
