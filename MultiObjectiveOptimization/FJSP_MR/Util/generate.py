...
# from ExactAlgorithm.Util import random_seed
from ExactAlgorithm.Util.read_mk_mr import readDataByFJS
import importlib

# importlib.reload(random_seed)
...
def generate_job_info(con_digit, job_list):
    ...
    for job in job_list:
        ...
        for op in job.op_list:
            ...
            # 一个工件的所有的pmin 加和
            p_min_sum += min(op.available_machines_am.values())
        job.release_time = random.randint(0, p_max)
        # job.release_time = 0
        job.delivery_time = con_digit * p_min_sum
...
def generate_reconfiguration_time(job_list, machine_list, auxiliary_module_list):
    ...
    #     # 装时间
    #     a = random.randint(p_max, 2.0 * p_max)
    #     # 拆时间
    #     b = round(random.uniform(0.8 * a, 1.2 * a))
...
def readDataByFJS1(fileNameFjs):
    ...
    for name in range(int(num_jobs)):
        ...
        for op_index in range(int(num_ops)):
            ...
            for i in range(int(num_available_machines)):
                op.available_machines_am.append((int(code[0]), int(code[1])))
                del code[0]
                del code[0]
            job.op_list.append(op)
