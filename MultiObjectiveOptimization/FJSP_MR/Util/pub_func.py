...
def get_y_factor(is_assemble, op, machine_list, auxiliary_module_list):
    return [-(get_process_time(op, machine, am) + (is_assemble * am.assemble_time[machine.name])) for machine in
            machine_list
            for am in auxiliary_module_list]


def get_reconfigure_time_τ(machine: Machine, _machine: Machine, am: AuxiliaryModules):
    return 0.0 if machine == _machine else float(am.disassemble_time[_machine.name] + am.assemble_time[machine.name])


def get_reconfigure_time_γ(machine: Machine, am: AuxiliaryModules, _am: AuxiliaryModules):
    return 0.0 if am == _am else float(am.assemble_time[machine.name] + _am.disassemble_time[machine.name])
