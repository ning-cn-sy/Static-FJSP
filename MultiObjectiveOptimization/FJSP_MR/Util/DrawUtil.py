...
def draw_gantt_chart(tasks, machine_list, auxiliary_module_list):
    ...
    machine = []
    am = []
    #   在ExactAlgorithm里别忘了改，之前用的是machine列表
    for task in tasks:
        machine.append(task.machine)
        am.append(task.am)
    ...
    for b in range(len(tasks)):
        ...
        assemble_time = tasks[b].am.assemble_time[tasks[b].machine.name]
        disassemble_time = tasks[b].am.disassemble_time[tasks[b].machine.name]
...
#   没用了在之前写的cplex 里用的draw_chart
def draw_chart(result, job_list, machine_list, auxiliary_module_list):
    print("aaa")
    # tasks_list = []
    # for i in range(len(result)):
    #     variable_name = spilt_variable_name(result, i)
    #     # print(variable_name)
    #     # 要C 对应完成时间 和  Y ==1  对应 op machine am duration
    #     if variable_name[0] == "C":
    #         op = job_list[int(variable_name[1]) - 1].op_list[int(variable_name[2]) - 1]
    #         # 先把其他的start time machine等随便赋值
    #         tasks_list.append(Task(op=op, start_time=-3.1, duration=-3.2, complete_time=result[i][1],
    #                                 machine=None, am=None))
    #     if variable_name[0] == "y" and result[i][1] > 0.9:
    #         # 找到tasks——list里的每一个task   通过tasks. 给他赋值
    #         # 通过job id  op id找对应的tasks   return
